...
$term = get_queried_object(); 
 		...
 		$parentid  =  isset($term->parent)? $term->parent :''; 

		echo '<div class="hide archivepage" data-parentid='.$parentid.'>';
			
			ij_shortcode_fetch_child_categories($parentid);
...
if ( ( function_exists( 'woocommerce_product_loop' ) && woocommerce_product_loop() ) || ( ! function_exists( 'woocommerce_product_loop' ) && have_posts() ) ) {
	...
	if ( ! $builder_id ) {
		...
		<div class="category-meta meta-hide-show ">
		<div class="category-desc short-desc mb-5" id="expand">
		    <?php 
		...
		<div class="category-desc full-desc mb-5" id="contract" style="display: none;">
		    <?php
		...
		//echo "<h2>Child Category</h2>";
		//ij_fetch_child_product_categories($term->parent);
		?>
		...
		if ( $skeleton_lazyload ) {
			...
			ij_k_get_child_categories($term->parent);
			$products_count = 0;
		}
		?>
		<?php
